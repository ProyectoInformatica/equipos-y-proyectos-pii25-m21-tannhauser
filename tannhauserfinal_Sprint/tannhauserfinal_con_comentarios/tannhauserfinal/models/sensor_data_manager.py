...
class SensorDataManager:
    def __init__(self):
        self.sensor_catalog = {
            ...
            "sensor_ordinario": {
                "type_code": "ORDINARY_SENSOR",
                "type_name": "Sensor ordinario",
                "unit": None,
                "category": "otro",
                "description": "Sensor ordinario que recoge un dato numerico y un dato alfanumerico",
                "device_code": "ordinary_01",
                "device_name": "Sensor ordinario principal",
                "location_name": "Zona general",
            },
        }
    ...
    def guardar_lectura_sensor_ordinario(
        self,
        valor_numerico,
        valor_alfanumerico: str,
        source: str = "esp32"
    ):
        """Guarda el sensor ordinario pedido en el ejercicio.

        El sensor genera dos datos en una misma lectura: uno numerico y otro
        alfanumerico. El dato numerico se guarda en reading_value y el dato
        alfanumerico se guarda en text_value. Ademas, ambos quedan reflejados
        en current_state para poder consultar el ultimo estado del dispositivo.
        """
        nombre_sensor = "sensor_ordinario"
        device_id, sensor_type_id, unit = self._ensure_sensor_resources(nombre_sensor)
        ...
        numeric_value = float(valor_numerico) if self._is_numeric(valor_numerico) else 0.0
        text_value = str(valor_alfanumerico or "")
        ...
        cursor.execute(
            """
            INSERT INTO readings (
                device_id, sensor_type_id, reading_value, text_value,
                normalized_value, consumption_w, reading_unit, reading_kind,
                recorded_at, source, payload
            )
            VALUES (%s, %s, %s, %s, %s, NULL, %s, 'mixto', NOW(6), %s, %s)
            """,
            (
                device_id,
                sensor_type_id,
    ...
    def leer_lecturas_sensor(self, nombre_sensor: str) -> List[Dict]:
        ...
        if nombre_sensor == "sensor_ordinario":
            cursor.execute(
                """
                SELECT reading_value, text_value, reading_kind, recorded_at
                FROM readings
                WHERE device_id = %s
            ...
            return [
                {
                    ...
                    "valor_numerico": float(row["reading_value"]),
                    "valor_alfanumerico": row.get("text_value"),
                    "tipo_dato": row.get("reading_kind") or "mixto",
                }
                for row in rows
            ]
