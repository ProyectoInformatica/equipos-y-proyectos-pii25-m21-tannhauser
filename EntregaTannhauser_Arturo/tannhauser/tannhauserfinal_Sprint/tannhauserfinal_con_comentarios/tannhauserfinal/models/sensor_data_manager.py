...
class SensorDataManager:
    ...
    def guardar_lectura_sensor(self, nombre_sensor: str, valor, source: str = "simulado"):
        ...
        # Soporta tres formas de `valor`:
        # 1) numérico simple -> guarda en `reading_value`
        # 2) texto simple -> guarda como estado en current_state
        # 3) compuesto (dict {'num':..., 'text':...} o [num, text]) -> guarda ambos en readings

        # Caso compuesto: dict con claves 'num'/'text' o lista/tupla [num, text]
        if isinstance(valor, dict) and ("num" in valor or "text" in valor) or (
            isinstance(valor, (list, tuple)) and len(valor) == 2
        ):
            if isinstance(valor, dict):
                num_val = valor.get("num")
                text_val = valor.get("text")
            else:
                num_val = valor[0]
                text_val = valor[1]

            numeric_value = float(num_val) if self._is_numeric(num_val) else None

            cursor = conn.cursor()
            cursor.execute(
                """
                INSERT INTO readings (
                    device_id, sensor_type_id, reading_value, reading_text, normalized_value,
            ...
            state_repr = str(text_val) if text_val is not None else (str(numeric_value) if numeric_value is not None else None)
            ...
            # Texto/string -> tratamos como estado
            state_code = "estado"
    ...
    def leer_lecturas_sensor(self, nombre_sensor: str) -> List[Dict]:
        ...
        cursor.execute(
            """
            SELECT reading_value, reading_text, recorded_at
            FROM readings
            WHERE device_id = %s
